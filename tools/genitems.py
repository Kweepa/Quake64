#!/usr/bin/env python3
"""Cook pickup line meshes from editor JSON → src/item_mesh.asm (unique X/Z)."""
...
def main() -> None:
    ...
    if DOC.exists():
        ...
        if isinstance(raw, dict):
            for k, v in raw.items():
                mesh = parse_mesh(v)
                if k == "backpack" and not mesh["verts"]:
                    continue
                items[k] = mesh if mesh["verts"] else items.get(k, {"verts": [], "lines": []})
    ...
    print(f"Wrote {OUT.relative_to(ROOT)}: backpack + {n_custom} custom type meshes")
