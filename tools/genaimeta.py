...
TWO_BYTE = {
    0x01, 0x05, 0x06, 0x09, 0x10, 0x11, 0x15, 0x16,
    0x21, 0x24, 0x25, 0x26, 0x29, 0x30, 0x31, 0x35, 0x36,
    0x41, 0x45, 0x46, 0x49, 0x50, 0x51, 0x55, 0x56,
    0x61, 0x65, 0x66, 0x69, 0x70, 0x71, 0x75, 0x76,
...
def main() -> None:
    ...
    for dos in DOS_NAMES:
        ...
        found += 1
    skel_path = ENEMY_DIR / "skel_draw.bin"
    if skel_path.is_file():
        code = skel_path.read_bytes()
        internal, map_sites = scan(code, link, sentinel)
        out = ENEMY_DIR / "skel_draw.rel"
        out.write_text(
            json.dumps(
                {"entry": 0, "internal": internal, "map": map_sites},
                indent=2,
        ...
        print(
            f"skel_draw: code={len(code)} internal={len(internal)} "
            f"map={len(map_sites)}"
        )
        found += 1
    if found == 0:
